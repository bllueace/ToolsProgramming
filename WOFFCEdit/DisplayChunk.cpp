...
DisplayChunk::DisplayChunk()
{
	...
	m_terrainSize = 512; // changes the plane size
...
void DisplayChunk::LoadHeightMap(std::shared_ptr<DX::DeviceResources>  DevResources)
{
	...
	////////////////////////////////////////////////
	//load paint texture 1
	std::wstring painttexture_01 = StringToWCHART(m_tex_splat_1_path);
	ps = CreateDDSTextureFromFile(device, painttexture_01.c_str(), NULL, &m_texture_paint_01);	//load tex into Shader resource	view and resource
	//////////////////////////////////////////////////////
...
void DisplayChunk::UpdateTerrain(Vector3 rOrigin, Vector3 rDirection)
{
	//int index;
	//-1 in order not to check the ends of arrays
	for (size_t i = 0; i < TERRAINRESOLUTION-1; i++)
	{
		for (size_t j = 0; j < TERRAINRESOLUTION-1; j++)
		{
			...
			if (RayIntersectsTriangle(rOrigin, rDirection, m_terrainGeometry[i][j].position,
				m_terrainGeometry[i][j+1].position, m_terrainGeometry[i+1][j+1].position,pick) ||
				RayIntersectsTriangle(rOrigin, rDirection, m_terrainGeometry[i][j].position,
					m_terrainGeometry[i+1][j + 1].position, m_terrainGeometry[i + 1][j].position, pick))
			{
				m_terrainGeometry[i][j].position.y		+= strength;
				m_terrainGeometry[i][j+1].position.y	+= strength;
				m_terrainGeometry[i+1][j+1].position.y	+= strength;
				m_terrainGeometry[i+1][j].position.y	+= strength;
			}
		}
	}
...
Vector3 DisplayChunk::updateObjectHeight(Vector3 originm, Vector3 direction)
{
	if (RayIntersectsTriangle(originm, direction, m_terrainGeometry[(int)originm.x][(int)originm.z].position,
		m_terrainGeometry[(int)originm.x][(int)originm.z+1].position, m_terrainGeometry[(int)originm.x+1][(int)originm.z+1].position, pick) ||
		RayIntersectsTriangle(originm, direction, m_terrainGeometry[(int)originm.x][(int)originm.z].position,
			m_terrainGeometry[(int)originm.x+1][(int)originm.z+1].position, m_terrainGeometry[(int)originm.x+1][(int)originm.z].position, pick))
	{
		col = pick;
	}
...
bool DisplayChunk::RayIntersectsTriangle(Vector3 rayOrigin,Vector3 rayVector,Vector3 v0, Vector3 v1, Vector3 v2,Vector3& outIntersectionPoint)
{
