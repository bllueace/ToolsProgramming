...
ToolMain::ToolMain()
{
	...
	////////////////////////////////////////
	m_toolInputCommands.RawMouseX      = 0;
	m_toolInputCommands.RawMouseY      = 0;
	m_toolInputCommands.mouse_X        = 0;
	m_toolInputCommands.mouse_Y        = 0;
	m_toolInputCommands.mouseDragX = 0.0f;
	m_toolInputCommands.mouseDragY = 0.0f;
	m_toolInputCommands.rightMouseDown = false;
	m_toolInputCommands.leftMouseDown  = false;
	m_toolInputCommands.showCursor = true;
	m_toolInputCommands.terrainPainting = false;
...
void ToolMain::onActionUndo()
{
	...
	m_d3dRenderer.undoLastAction();
}
void ToolMain::onActionRedo()
{
	//m_toolInputCommands.Undo = true;
	m_d3dRenderer.redoLastAction();
}

void ToolMain::onActionSculpting()
{
	if(!m_toolInputCommands.terrainPainting)
	{
		m_toolInputCommands.terrainPainting = true;
		m_toolInputCommands.rotateMode = false;
		m_toolInputCommands.scaleMode = false;
	}
	else if(m_toolInputCommands.terrainPainting)
	{
		m_toolInputCommands.terrainPainting = false;
	}
}
...
void ToolMain::onActionRotate()
{
	if (!m_toolInputCommands.rotateMode)
	{
		m_toolInputCommands.rotateMode = true;
		m_toolInputCommands.terrainPainting = false;
		m_toolInputCommands.scaleMode = false;
	}
	else if (m_toolInputCommands.rotateMode)
	{
		m_toolInputCommands.rotateMode = false;
	}
}
...
{
	if(!m_toolInputCommands.scaleMode)
	{
		m_toolInputCommands.scaleMode = true;
		m_toolInputCommands.terrainPainting = false;
		m_toolInputCommands.rotateMode = false;
	}
	else if(m_toolInputCommands.scaleMode)
	{
		m_toolInputCommands.scaleMode = false;
	}
}
...
void ToolMain::Tick(MSG *msg)
{
	...
	if (!m_toolInputCommands.terrainPainting)
	{
		if (m_toolInputCommands.leftMouseDown)
		{
			...
			//m_d3dRenderer.TerrainSculptingCheck();
			//m_toolInputCommands.leftMouseDown = false;
		}
	}
	if (m_toolInputCommands.terrainPainting)
...
void ToolMain::UpdateInput(MSG * msg)
{
	...
	case WM_KEYDOWN:
		...
		m_toolInputCommands.buttonDown = true;
		break;

	case WM_KEYUP:
		m_keyArray[msg->wParam] = false;
		m_toolInputCommands.buttonDown = false;
		break;
	...
	case WM_MOUSEMOVE:
		...
		if (m_toolInputCommands.rightMouseDown)
		{
			...
			m_toolInputCommands.mouseDragX = ((2.0f * (float)m_toolInputCommands.mouse_X) / (float)m_width) - 1.0f;
			m_toolInputCommands.mouseDragY = (((2.0f * (float)m_toolInputCommands.mouse_Y) / (float)m_height) - 1.0f) * -1.0f;
		}
		break;
	...
	case WM_RBUTTONDOWN:	//mouse button down,  you will probably need to check when its up too
		...
		clientCenter.x = rect.right / 2;
		clientCenter.y = rect.bottom / 2;

		ClientToScreen(msg->hwnd, &clientCenter);
	...
	case WM_INPUT:
		...
		GetRawInputData(reinterpret_cast<HRAWINPUT>(msg->lParam), RID_INPUT, NULL, &dataSize, sizeof(RAWINPUTHEADER)); //Need to populate data size first
		...
		if (dataSize > 0)
		{
			...
			if (GetRawInputData(reinterpret_cast<HRAWINPUT>(msg->lParam), RID_INPUT, rawdata.get(), &dataSize, sizeof(RAWINPUTHEADER)) == dataSize)
			{
				...
				{
					m_toolInputCommands.RawMouseX += raw->data.mouse.lLastX;
					m_toolInputCommands.RawMouseY += raw->data.mouse.lLastY;
				}
			}
		}
		break;
	...
	if (m_keyArray['F'])
	{
		m_toolInputCommands.objFocus = true;
	}
	else m_toolInputCommands.objFocus = false;
	if (m_keyArray['C'])
	{
		m_toolInputCommands.Undo = true;
	}
	else m_toolInputCommands.Undo = false;
	if (m_keyArray['T'])
	{
		m_toolInputCommands.showCursor = false;
	}
	else m_toolInputCommands.showCursor = true;
